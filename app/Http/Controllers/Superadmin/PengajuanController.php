...
class PengajuanController extends Controller
{
    public function index()
    {
       ...
       // ->select('status')->first();
        //return $daftars;
       return view('superadmin.pengajuan.pengajuan',compact('pengajuans'));        
    }
    ...
    public function editStat($id)
{
    ...
    return view('superadmin.pengajuan.pengajuan', ['id' => $id]);
}
    ...
    public function statusUpdate(Request $request,$id=0){    
        ...
        if(count($a)>2){
            ...
            return redirect('superadmin/pengajuan')->with('status', 'Asisten Praktikum Sudah Cukup!');
        }else{
        if($daftars->status === "daftar" || $daftars->status === "ditolak"){
            ...
            return redirect('superadmin/pengajuan')->with('status', 'Berhasil Menerima Asistensi');
            ...
            Session::flash('statuscode','success');
            return redirect('superadmin/pengajuan')->with('status', 'Berhasil Menolak Asistensi');
        }
    }
    }
}
